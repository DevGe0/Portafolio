...
$('#buscar_pais').on('click', ()=> {
  ...
  var urlsearch = `https://api.openweathermap.org/data/2.5/weather?q=${pais}&appid=${key}&lang=es&units=metric`
...
function weatherReport(data){
  ...
  var urlcast= `https://api.openweathermap.org/data/2.5/forecast?q=${data.name}&appid=${key}&lang=es&units=metric`
  ...
  fetch(urlcast).then((res)=>{
    ...
    let contenedor = (`<div class="contenedor_datos row">`)
        ...
        for (let i=8; i<forecast.list.length; i+=8){
          ...
          var Fecha = new Date(forecast.list[i].dt*1000);
          const meses = ["JAN", "FEB", "MAR","APR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DEC"];
          const formatFecha = (fecha) => {
        ...
        for (let j=0; j<forecast.list.length-31; j++){
          ...
          var Hora = new Date(forecast.list[j].dt*1000);
